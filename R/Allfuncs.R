...
#' @title solveU
#' @description Solves for the U coefficients making efficient utilization of 
#' the lasso path
#' 
#' @param Z current Z estimate
#' @param Chat the inverse of the C matrix
#' @param priorMat the prior pathway or C matrix
#' @param penalty.factor Penalties for different pathways, must have size 
#' ncol(priorMat).
...
#' @title plierResToMarkers
#' @description Creates a binary cell-type marker matrix using prior results. 
#' This matrix can be used for other downstream tasks that require
#' cell-type markers, such as CellCODE
#' 
#' @param plierRes A PLIER result
#' @param priorMat the binary prior information matrix that was used to compute 
#' the plierResult. Including this insures that only the genes annotated to the 
#' pathway(s) are included
#' @param num The number of marker genes to produce
#' @param index The indecies of PLIER latent variables that are believed to 
#' represent cell-type proportions (as opposed to other sources of correlation)
#' 
#' @export
#' 
...
#' @title combinePaths
#' @description combines binary pathway matricies into one, rownames are 
#' matched by name
#'
...
#' @title getCutoff
#' @description get the p-value cutoff for a specific FDR
#'
#' @param plierRes A PLIER result
#' @param fdr.cutoff The cross-validation significance cutoff for a pathway to 
...
#' @title nameB
#' @description names latent variables according to their pathway useage (if
#' any)
#'
#' @param plierRes A PLIER result
#' @param top The number of pathway to use. Only the top pathway (one with the 
#' largest coefficient) is used by default
#' @param fdr.cutoff The cross-validation significance cutoff for a pathway to 
#' be considered for naming. If no pathways satisfy the cutoff the raw 
#' coefficients are used.
#' @param Choice of coef or AUC, whether LVs are named based on U coefficients 
#' or AUCs. Defualt: coef.
#' 
#' @export
#' 
...
#' @title computeChat
#' @description Computes the ridge pseudo-inverse of the prior information 
#' matrix. Used internally by PLIER but can be precomputed if running PLIER 
#' multiple times.
#'
#' @param gsMat The prior information matrix. The genes have to match the gene 
#' expression data to be analyzed exactly (same genes and same order(
...
#' @title PLIER
#' @description Main PLIER function
#'
#' @param data the data to be processed with genes in rows and samples in 
#' columns. Should be z-scored or set scale=T
#' @param priorMat the binary prior information matrix with genes in rows and 
#' pathways/genesets in columns
#' @param svdres Pre-computed result of the svd decomposition for data
#' @param k The number of latent variables to return, leave as NULL to be set 
#' automatically using the num.pc "elbow" method
...
#' @param L3 L3 constant, leave as NULL to automatically select a value. 
#' Sparsity in U should be instead controlled by setting frac
#' @param frac The fraction of LVs that should have at least 1 prior 
#' inforamtion association, used to automatically set L3
...
#' @param Chat A ridge inverse of priorMat, used to select active pathways, 
#' expensive to compute so can be precomputed when running PLIER multiple times
#' @param maxPath The maximum number of active pathways per latent variable
#' @param doCrossval Whether or not to do real cross-validation with held-out 
#' pathway genes. Alternatively, all gene annotations are used and only 
#' pseudo-crossvalidation is done. The latter option may be preferable if some 
#' pathways of interest have few genes.
#' @param penalty.factor A vector equal to the number of columns in priorMat. 
#' Sets relative penalties for different pathway/geneset subsets. Lower 
#' penalties will make a pathway more likely to be used. Only the relative 
#' values matter. Internally rescaled.
#' @param glm_alpha Set the alpha for elastic-net
#' @param minGenes The minimum number of genes a pathway must have to be 
#' considered
#' @param tol Convergence threshold
#' @param seed Set the seed for pathway cross-validation
#' @param allGenes Use all genes. By default only genes in the priorMat 
#' matrix are used.
#' @param rseed Set this option to use a random initialization, instead of SVD
#' @param pathwaySelection Pathways to be optimized with elstic-net penalty are 
#' preselected based on ridge regression results. "Complete" uses all top 
#' pathways to fit individual LVs. "Fast" uses only the top pathways for the 
#' single LV in question.
#' 
#' @export
#' 
PLIER <- function(data, 
...
#' @title plotU
#' @description plot the U matrix from a PLIER decomposition
#'
#' @param plierRes the result returned by PLIER
#' @param auc.cutoff the AUC cutoff for pathways to be displayed, increase to 
#' get a smaller subset of U
...
#' @param indexRow restrict to a subset of rows (pathways). Useful if only 
#' interested in pathways of a specific type
#' @param top the number of top pathways to discplay for each LV
#' @param sort.row do not custer the matrix but instead sort it to display the 
#' positive values close do the 'diagonal'
#' @param ... options to be passed to pheatmap
#' 
#' @export
#' 
plotU <- function(plierRes,
                  auc.cutoff = 0.6,
                  fdr.cutoff = 0.05, 
                  indexCol = NULL, 
                  indexRow = NULL, 
...
#' @title plotTopZ
#' @description visualize the top genes contributing to the LVs
#'
#' @param plierRes the result returned by PLIER
#' @param data the data to be displayed in a heatmap, typically the z-scored
#' input data (or some subset thereof)
#' @param priorMat the same gene by geneset binary matrix that was used to run 
#' PLIER
#' @param top the top number of genes to use
#' @param index the subset of LVs to display
#' @param regress remove the effect of all other LVs before plotting top genes, 
...
#' @title commonRows
#' @description returns the row names in common
#' 
...
#' @title rowNorm
#' @description z-score each row of the data
#' 
...
#' @title num.pc
#' @description estimates the number of 'significant' principle components for
#' the SVD decomposition -- this is the minimum k for PLIER
#' @param data the same data as to be used for PLIER (z-score recommended) or
#' alternatively the result of an svd calculation
#' @param method Either "eblow" (fast) or "permutation" (slower, but less
#' heuristic)
...
#' @title plotMat
#' @description generic function to plot the non-zero elements of a sparse
#' matrix
#'
...
#' @title plotTopZallPath
#' @description visualize the top genes contributing to the LVs similarily to
#' [plotTopZ()]. However in this case all the pathways contributing
#' to each LV are show seperatly. Useful for seeing pathway usage for a single
#' LV or understading the differences between two closely related LVs
#'
#' @param plierRes the result returned by PLIER
#' @param data the data to be displayed in a heatmap, typically the z-scored
#' input data (or some subset thereof)
...
#' @export
#'
plotTopZallPath <- function(plierRes, 
                            data, 
                            priorMat, 
                            top = 10, 
                            index = NULL, 
  ...
  for (i in 1:length(ii)) {
    ...
    nncol <- c(nncol, rep(rownames(plierRes$U)[which(thispath <- plierRes$U[
      ,
      ii[i]
    ] == max(plierRes$U[, ii[i]]))], length(nntmp)))
    nnindex <- c(nnindex, rep(ii[i], length(nntmp)))
  ...
  pheatmap(tscale(toPlot),
    breaks = bb, color = colorpanel(
      101,
      "green", "white", "red"
    ), annotation_row = as.data.frame(pathMat), annotation_legend = F, show_colnames = F, annotation_colors = anncol,
...
resid <- function(dat, lab, useMean = T) {
  ...
  if (!is.null(ne)) {
    cat(
      "Coefficients not estimable:", paste(ne, collapse = " "),
      "\n"
    )
...
#' @title PLIERsparse
#' @description sparse PLIER function (experimental)
#'
#' @param data the data to be processed with genes in rows and samples in
#' columns. Should be z-scored or set scale=T
#' @param priorMat the binary prior information matrix with genes in rows and
...
#' @param L3 L3 constant, leave as NULL to automatically select a value.
#' Sparsity in U should be instead controlled by setting frac
#' @param frac The fraction of LVs that should have at least 1 prior inforamtion
#' association, used to automatically set L3
#' @param max.iter Maximum number of iterations to perform
#' @param trace Display progress information
#' @param scale Z-score the data before processing
#' @param Chat A ridge inverse of priorMat, used to select active pathways,
...
#' @param doCrossval Whether or not to do real cross-validation with held-out
#' pathway genes. Alternatively, all gene annotations are used and only
#' pseudo-crossvalidation is done. The latter option may be preferable if
#' some pathways of interest have few genes.
#' @param penalty.factor A vector equal to the number of columns in priorMat.
#' Sets relative penalties for different pathway/geneset subsets. Lower
#' penalties will make a pathway more likely to be used. Only the relative
#' values matter. Internally rescaled.
...
#' @param tol Convergence threshold
#' @param seed Set the seed for pathway cross-validation
#' @param  allGenes Use all genes. By default only genes in the priorMat matrix
#' are used.
#' @param rseed Set this option to use a random initialization, instead of SVD
#' @param pathwaySelection Pathways to be optimized with elstic-net penalty are
#' preselected based on ridge regression results. "Complete" uses all top
